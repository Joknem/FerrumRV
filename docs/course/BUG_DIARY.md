...
## Milestone 4.1：遗漏 PC 更新与测试观察不足

- Symptom: 初始 execute 只写寄存器，没有更新 PC；旧测试仍通过，因为没有调用 execute。
- Wrong hypothesis: 未记录学生独立提出的错误假设。
- Observation: 教师源码检查发现遗漏；后续测试曾只检查调用成功或 PC，没有检查连续程序的运算结果，另有一处 halted 断言引用了错误 CPU 对象。
- Root cause: 实现遗漏状态更新；测试未完整观察本次行为的结果。
- Fix: 学生添加 PC 回绕更新、修正断言对象，补齐三步的 x1/x2/PC 对照及失败状态检查；十个测试回归通过。
- What I learned: Ok(()) 不能证明计算结果正确，旧测试通过也不证明新增方法已被覆盖；逐步比较状态可以暴露首次偏差。
