...
- 当前 Phase：A
- 当前 Chapter：5
- 当前 Milestone：5.3 — JALR
- 已通过 milestone：0.1、0.2、1.1、1.2、2.1、2.2、3.1、3.2、3.3、4.1、4.2、4.3
- 当前状态：5.2 已实现，十五个测试及格式检查通过，无警告；待学生提交。
- 当前待验收项：本章 5.2 无；JALR 的 I-type 偏移、链接寄存器和目标地址对齐尚未开始。
- 下一目标：开始 5.3 JALR，连接间接跳转与函数调用/返回。

## Milestone 5.2 学习记录

- PASS：JAL 使用 J-type 立即数，保存旧 PC+4 到 rd，再以旧 PC+imm 更新 PC；支持正负偏移与 rd=x0。
- Tested：立即数正负边界、链接地址、负偏移循环、有限步数、EBREAK 停机和最终寄存器/PC 均通过。十五个测试与格式检查通过，无警告；移除了未使用导入和预留 JALR 常量。
- 当前边界：尚未实现 JALR；JAL 测试中的循环仍由固定步数上限保护。
...
- 学生实现体现协议：EBREAK 只设置 halted=true，PC 保留在当前指令；step 在 halted 时返回 Ok(())，不取指。此为教学 host 停止约定，不是真实断点异常处理。
- 解码已修正：opcode=0x73 与 raw>>7=0x2000 联合检查等价于完整匹配0x00100073；ECALL及分别改动rd/rs1/funct3的相近编码均返回错误。
- 最新测试：循环上限10，检测 halted 后 break；算术结果与停止 PC 正确。停止后将当前指令替换为 SUB，再调用 step，比较整个寄存器数组、PC和halted不变。该 SUB 若执行会把x3从10改为7并推进PC，能发现缺少停止检查的问题，无需指定必须使用非法编码。十二个测试和格式检查通过，无警告。
- 章末解释通过：学生说明真实 EBREAK 用于把控制权交还执行环境，让执行环境进行后续处理并中断当前程序；`halted=true` 是教学模拟器自己的简化约定，不是 EBREAK 的完整 RISC-V 语义。

## Milestone 4.2 学习记录

- 最终验收 PASS：ADD/SUB 按完整 funct7/funct3 区分；普通运算、负结果、回绕、源目标重合及 x0 写保护通过。写 x0 后 PC/halted 断言已补齐；四条指令逐步验证 PC、x3=[0,0,13,10]、halted，最终 x0=0。
- 两类不支持 OP 编码 0x007312b3、0x027302b3 均返回带 PC 与原始机器码的 StepError::Decode。此前全部测试恢复并通过。
- 理论与 Rust：围绕学生问题讲解 opcode 类别与格式的区别、R-type 字段、u32 保存补码位模式、wrapping_sub，以及 if/match 表达式和 todo! 的覆盖范围；学生将这些用于实现、负值断言和逐步预期数组。Debuggable：识别并移除核心错误路径占位宏，恢复被禁用的回归测试；Integrated：十一个测试与格式检查通过。
- 容量参数由学生实现，教师只做授权的调用点迁移。以下旧记录保留为过程记录，以本段最终验收为准。

- 最新复验：学生完成容量参数、解码错误返回以及 ADD 0xffffffff+1 和 SUB 0-1 用例。十一个测试全部通过，无警告。四步程序仍只检查最终寄存器，未加入逐步 PC、第三步 x3=13 及最终 x0/halted 断言；R-type 两类错误字段测试尚缺。

- 学生选择 Memory::new 按实例指定容量，要求减少机械迁移。教师仅批量迁移七处测试调用：旧测试用16，新 ADD/SUB 测试用256，并恢复六个旧测试属性，未修改核心构造函数或新增测试逻辑。待学生为构造函数添加 usize 容量参数并用于分配；当前调用方已先迁移，暂不能编译。此阶段容量约定仍为非零且不跨 guest 地址空间末端，不声称支持任意 usize。

- 本轮 review：读取实现确认 ADD/SUB 先读两个源再写回、wrapping 运算及 PC 更新正确。OP 分支未知 funct7/funct3 仍为 todo!()，会触发 host panic，必须返回结构化错误。
- 学生将 RAM 从 16 改为 256 字节并注释 fetch_test、inst_test、step_test 及三个 Memory 测试。当前 cargo test 仅五个用例通过，不证明旧行为回归；容量变化会使旧边界期望失效。建议本轮按独立用例复用 16 字节 RAM；如保留扩容，须同步所有边界测试而不能禁用。
- 已有新测试覆盖普通 ADD/SUB、编号31及源目标重合、写x0、四步最终结果；尚缺 R-type 非支持字段错误、ADD 正向回绕和 SUB 0-1、相应 PC/halted 断言及四步中间状态检查。

- 本轮任务包含类型、解码、执行及测试，不拆分签名验收。重点检查 SUB 操作数顺序、funct7 严格识别、ADDI 立即数位不能套用 R-type 限制，以及 PC 每条只更新一次。

## 教学偏好（2026-09-20 更新）

- 学生明确要求提高任务粒度、补足 Rust 和计算机原理讲解。遵循 FerrumRV_COURSE.md 第 21 节新约定：原理说明 → 完整小任务及验收范围 → 学生独立实现 → 集中 review。
- 不默认拆分签名、占位、函数体；不逐个追加测试或重复已掌握知识；必要基础讲解不受 Hint 等级限制。仍不代写核心代码。

## Milestone 4.1 学习记录

- PASS / Works：完成 fetch → decode → execute → 下一状态的单步闭环。
- Tested：普通 ADDI、正负回绕、源目标相同、写 x0 与 PC 回绕；取指/解码失败均验证错误上下文及 CPU 状态保持；三条连续 ADDI 每步检查 x1、x2 和 PC，最终检查 x0 与 halted。
- Explained：学生解释 x1 在第一条写入后保持为 10，第三条读取同一 CPU 的当前状态；此前正确推演 PC、x0 和回绕结果。
- Debuggable：经历 PC 更新遗漏、未断言程序结果、断言引用错误 CPU 对象等问题；最终通过逐步状态比较定位偏差。Integrated：十个测试通过，cargo fmt --check 通过，无测试编译警告。
- 当前边界：停止行为留待 4.3；step 调用前 CPU 未停止、PC 对齐仍为前提。三条程序由测试控制执行次数，不表示 guest 执行了停止指令。机器码按学生明确请求由教师提供，核心代码及测试由学生编写。

- 开章已读取当前 CPU 实现与教案；尚未实现执行逻辑。先讨论普通 ADDI 和写 x0 的状态变化，不直接提供核心实现。
- 学生正确推演普通执行、写 x0 仍前进 PC、0xffffffff+1 回绕为零以及 0 加 -1 得到 0xffffffff。
- execute 使用 &mut self，先读取再 wrapping_add，通过 wreg 写回，PC 使用 wrapping_add(4)。曾遗漏 PC 更新及其断言，已亲手补齐。
- 最新测试：普通负立即数执行、源目标同为 x6 的正向回绕、负向回绕、写 x0 与 PC 回绕组合用例均通过，八个测试与格式检查通过。
- 学生已实现 StepError::Fetch/Decode 与 step；取指和解码用 match 包装原始错误及当前 PC，成功才执行，PC 仅由 execute 更新。
- 最新单步验证：成功结果和 PC/halted 正确；取指与解码失败均比较全部寄存器、PC、halted。准备内存的 Result 均已检查。
